...
def push_to_hub(model, source_model, tokenizer, repo_id: str):
    ...
    stats = [
        {
            "model": source_model,
            "parameter_size_full": pretty(
                model.config.vocabtrimmer["stats"]["parameter_size_full/raw"]
            ),
            "parameter_size_embedding": pretty(
                model.config.vocabtrimmer["stats"]["parameter_size_embedding/raw"]
            ),
        ...
        },
        {
            "model": repo_id,
            "parameter_size_full": pretty(
                model.config.vocabtrimmer["stats"]["parameter_size_full/trimmed"]
            ),
            "parameter_size_embedding": pretty(
                model.config.vocabtrimmer["stats"]["parameter_size_embedding/trimmed"]
            ),
            "vocab_size": pretty(
                model.config.vocabtrimmer["stats"]["vocab_size/trimmed"]
            ),
            "compression_rate_full": round(
                model.config.vocabtrimmer["stats"]["compression_rate_full"], 2
            ),
            "compression_rate_embedding": round(
                model.config.vocabtrimmer["stats"]["compression_rate_embedding"], 2
            ),
        },
    ]
    ...
    readme = (
        f"# Vocabulary Trimmed [{source_model}]({link}): `{repo_id}` \n"
    ...
    raise NotImplementedError(
        "push_to_hub requires huggingface_hub>=0.8 with Repository support"
    )
    safe_rmtree(os.path.basename(repo_id))
...
class VocabTrimmer:
    ...
    def show_parameter(self, log: bool = False):
        return show_parameter(
            self.model, double_embedding=self.double_embedding, log=log
        )
    ...
    def trim_vocab(
        ...
        language: str,
        path_to_save: str,
        dataset: str = "vocabtrimmer/mc4_validation",
        dataset_column: str = "text",
        dataset_name: str = None,
        dataset_split: str = "validation",
        tokens_to_keep: List = None,
        ...
        dataset_name = (
            language
            if dataset in ["mc4", "vocabtrimmer/mc4_validation"]
        ...
        self.model.set_input_embeddings(
            torch.nn.Embedding.from_pretrained(input_embedding.weight[new_vocab_id])
        )
        ...
        self.model = self.__model_class.from_pretrained(
            path_to_save, config=self.config, ignore_mismatched_sizes=True
        )
        ...
        stats = {
            ...
            "compression_rate_full": self.param_size_full_trimmed
            / self.param_size_full_raw
            * 100,
            "compression_rate_embedding": self.param_size_embedding_trimmed
            / self.param_size_embedding_raw
            * 100,
        }
